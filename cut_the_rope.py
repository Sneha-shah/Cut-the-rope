...
hs = pygame.image.load('highscores.png')
...
hat1 = pygame.transform.scale(hat1,(int(candy_width*3),int(candy_width*3)))
hat2 = pygame.transform.scale(hat2,(int(candy_width*3),int(candy_width*3)))
hat1 = pygame.transform.rotate(hat1,170)
hs= pygame.transform.scale(hs,(int(display_h*0.7),int(display_h*0.8)))
...
def Menu():
    ...
    if pygame.mixer.music.get_busy():
        ...
        surface.blit(sound_off,(int(display_h/2.425373),int(display_h/1.340206)))
    pygame.display.update()
    global sound_state
    c = 1
    while c:
        for event in pygame.event.get():     
            if event.type == pygame.QUIT:
                ...
                if int(mx) in range(int(display_h/2.32142857),int(display_h/1.3)) and int(my) in range(int(display_h/1.3131313),int(display_h/1.17117117)):
                    Sound() # calling 001
                elif int(mx) in range(int(display_h/2.32142857),int(display_h/1.3)) and int(my) in range(int(display_h/1.547619),int(display_h/1.35416667)):
                    c = 0
                    break
                elif int(mx) in range(int(display_h/1.22),int(display_h/0.87)) and int(my) in range(int(display_h/1.547619),int(display_h/1.35416667)):
                    surface.blit(hs,(int(display_h*0.6-display_h*0.35),display_h*0.1))
                    fo=open('Highscores.DAT','rb')
                    L=[]
                    while 1:
                        try:
                            L+=[pickle.load(fo)]
...
def disphs(L):
    font_size = pygame.font.Font('freesansbold.ttf',int(candy_width*0.8))
    h = 200
    string = '%15s   %5s     %5s'%('NAME'.ljust(15),'LEVEL','SCORE')
    high = font_size.render(string, True, white)
    Rect = high.get_rect()
    Rect.center = (int(display_w/2), int(h))
    surface.blit(high, Rect)
    for i in L:
        h += candy_width
        string = '%15s  %2d         %4d'%(i.name.ljust(15),i.level,i.score)
        high = font_size.render(string, True, white)
        Rect = high.get_rect()
        Rect.center = (int(display_w/2), int(h))
        surface.blit(high, Rect)
    ...
    while c:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                (mx,my) = pygame.mouse.get_pos()
                if int(mx) in range(int(display_h*0.31),int(display_h*0.44)) and int(my) in range(int(display_h*0.81),int(display_h*0.88)):
                    c = 0
                    break
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if int(mx) in range(int(display_h/2.15),int(display_h/1.36)) and int(my) in range(int(display_h/5),int(display_h/3.6)):
                    ...
                    level = l1
                    l1.Play_Level()
                elif int(mx) in range(int(display_h/2.15),int(display_h/1.36)) and int(my) in range(int(display_h/2.5),int(display_h/2.07)):
                    l2.time_start = time.time()
                    level = l2
                    l2.Play_Level()
                elif int(mx) in range(int(display_h/2.15),int(display_h/1.36)) and int(my) in range(int(display_h/1.67),int(display_h/1.46)):
...
def fall((x,y),angle, v, a, dt, t,x_bowl,y_bowl, xc, yc, xc2=0, yc2=0, r2=0):
    ...
    while cont:
        ...
        for h in (h1,h2,hi1,hi2):
            if h.exist:
                if int(x) in range(h.pos1[0]-candy_width,h.pos1[0]+candy_width*3):
                    if int(y) in range(h.pos1[1]-candy_width,h.pos1[1]+candy_width*3):
                        if h.d1 == 'u' and v[1]>0:
                            surface.blit(h.img1,h.pos1)
                            pygame.display.update()
                            if t>t_h+0.3 and int(x) in range(int(h.pos1[0]-candy_width*0.2),int(h.pos1[0]+candy_width*2.2)):
                                if int(y) in range(h.pos1[1],h.pos1[1]+candy_width*2):
                                    t_h = t
                                    if h.d2 == 'u':
                                        (x,y) = h.pos2[0]+candy_width,h.pos2[1]
                                        v[0],v[1] = 0,-v[1]
                                    elif h.d2 == 'l':
                                        (x,y) = h.pos2[0],h.pos2[1]+candy_width
                                        v[0],v[1] = -v[1],0
                                    elif h.d2 == 'r':
                                        (x,y) = h.pos2[0]+candy_width*3,h.pos2[1]+candy_width
                                        v[0],v[1] = v[1],0
                                    else:
                                        (x,y) = h.pos2[0]+candy_width,h.pos2[1]+candy_width*3
                                        v[0],v[1] = 0,v[1]
                        elif h.d1 == 'l' and v[0]>0:
                            surface.blit(h.img1,h.pos1)
                            pygame.display.update()
                            if t>t_h+0.3 and int(x) in range(h.pos1[0],h.pos1[0]+candy_width*2):
                                if int(y) in range(int(h.pos1[1]-candy_width*0.2),int(h.pos1[1]+candy_width*2.2)):
                                    t_h = t
                                    if h.d2 == 'u':
                                        (x,y) = h.pos2[0]+candy_width,h.pos2[1]
                                        v[0],v[1] = 0,-v[0]
        ...
        #check for bowl
        for i in range(int(x_bowl+candy_width*0.2),int(x_bowl+candy_width*2.8)):
            if i==int(x):
                for i in range(int(y_bowl+candy_width*0.2),int(y_bowl+candy_width)):
                    if i==int(y):
                        cont=False
                        Result(True)
...
def render_textrect(string, font, rect, text_color, background_color, justification=0):
    #Returns Failure - raises a TextRectException if the text won't fit onto the surface.
...
def get_name():
    ...
    while c:
        surface.blit(back,(0,0))
        s = 'HIGHSCORE!! Please enter your name: '
        high = font_size.render(s+n, True, black)
        Rect = high.get_rect()
        Rect.center = (int(display_w/2), int(display_h/2-candy_width))
        surface.blit(high, Rect)
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
...
class score:
    ...
    def check_high(self):
        ...
        for i in range(mn-1,mx-1,-1):
                if L[i].score>self.score:
                    print 1
                    if L[i+1].score<self.score:
                        print 2
                        self.name = get_name()
                        del L[mn]
                        L.insert(i+1,self)
...
l2=levels(2,740-candy_width*2,600,450,250,700,135,345,252,677,406,0,0,0,0,'','',0,0,0,0,'','',200,50,200,0,0,0)
#l3=levels(3,400,500,660,100,200,100,460,300,650,500,200,350,500,400,'d','l',200,600,650,400,'u','u',200,200,150,0,0,0)
#l3=levels(3,500,600,330,270,700,100,550,450,260,600,250,400,650,400,'d','l',650,600,500,300,'u','d',200,200,150,0,0,0)
l3=levels(3,450,600,280,270,630,100,500,450,210,600,190,400,650,400,'d','l',580,600,450,300,'u','d',150,200,150,0,0,0)
angle_max  = (math.pi/2)
check=0
level = None
...
rendered_text = render_textrect(r, font_size, Rect, red, orange, 1)
